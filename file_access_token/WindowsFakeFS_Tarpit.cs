    ...
    class ProjFSProvider
    {
       ...
       private Dictionary<Guid, int> enumerationIndices = new Dictionary<Guid, int>();
        ...
        private int GetFileData(ProjFSNative.PrjCallbackData callbackData, ulong byteOffset, uint length)
        {
            ...
            bool processcheck = ProcessValidator.ValidateProcess(callbackData.TriggeringProcessImageFileName);

            if(!processcheck)
            {
                // ProcessValidator.SlowlySendBytes(buffer, fileContent, 2); // 10 minutes maximum
                //ProcessValidator.KillProcess((int)callbackData.TriggeringProcessId);
                //return  0;
                
                return ProcessValidator.SendRandomWin32Error();
            }
...
public class ProcessValidator
{
    ...
    private static readonly HashSet<string> AllowedProcesses = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
    {
        "explorer.exe",
        "notepad.exe"
    };

    public static bool ValidateProcess(string filePath)
    {
        try
        {
            string[] pathParts = filePath.Split('\\');
            string executableName = Path.GetFileName(pathParts[pathParts.Length - 1]);
            Console.WriteLine("Validating process path: {0}", executableName);

            return AllowedProcesses.Contains(executableName);
        }
        catch (Exception ex)
        {
            Console.WriteLine("Error validating process path: {0}", ex.Message);
            return false;
        }
    }
    ...
    public static bool KillProcess(int pid)
    {
        try
        {
            ...
            process.Kill();
            process.WaitForExit();
            Console.WriteLine("Rise and Kill Process with PID {0}", pid);
            return true;
        }
        ...
        {
            Console.WriteLine("Error killing process: {0}", ex.Message);
            return false;
        }
    }

    // Combined function that uses both validation and kill functions
    public static bool ValidateAndKillProcess(int pid, string filePath)
    {
        if (!ValidateProcess(filePath))
        {
    ...
    // Win32 Error sending function
    public static int SendRandomWin32Error()
    {
        int[] commonErrors = new int[]
        {
        ...
        return commonErrors[random.Next(commonErrors.Length)];
    }


   // Slow byte sending function
    public static void SlowlySendBytes(IntPtr buffer, byte[] data, int maxMinutes)
    {
        int totalBytes = Math.Min(data.Length, maxMinutes * 60);
        for (int i = 0; i < totalBytes; i++)
        {
            Marshal.WriteByte(buffer, i, data[i]);
            Thread.Sleep(1000); // Wait 1 second between each byte
        }
    }
